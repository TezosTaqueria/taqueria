import { EphemeralState, LoadedConfig, Task } from './taqueria-protocol/taqueria-protocol-types.ts';
...
export const generateProvisionTypes = (state: EphemeralState.t, config: LoadedConfig.t) =>
	(cliConfig: CLIConfig) => {
...
`,
		...
		const toPluginInterface = (plugin: typeof state.plugins[number]): [string, string, string, string] => {
		...
		const toEnvInterface = (envName: string): [string, string, string] => [
			...
			// export type TestingState = typeof testing_state
			// type RawState = DevelopmentState | TestingState
		...
		const configs = config.environment
			? Object.keys(config.environment).map(toEnvInterface)
