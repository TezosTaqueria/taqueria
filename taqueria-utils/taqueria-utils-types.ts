...
export class SanitizedAbsPath {
    ...
    protected constructor(value: string, cwd?: SanitizedAbsPath) {
        this.value = cwd ? resolvePath(cwd.join(value).value) : resolvePath(value)
    }
...
export class SHA256 extends StringLike {
    ...
    static create(value: string): SHA256 | undefined {
        return value.length === 64 ? new SHA256(value) : undefined
    }
    ...
    static futureOf(value: string): Future<TaqError, SHA256> {
        return attemptP(() =>
            SHA256.of(value).then((result) =>
                result
                    ? result
                    : Promise.reject({ kind: 'E_SHA256', message: 'Could not create hash', context: value }),
            ),
        )
    }
}
