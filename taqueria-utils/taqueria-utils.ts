...
export const writeTextFile =
    (path: string) =>
    (data: string): Future<TaqError, string> =>
...
export const toPromise = <T>(f: Future<TaqError, T>) =>
    pipe(
        f,
...
export const inject = (deps: UtilsDependencies) => {
    ...
    const gitClone =
        (url: SanitizedUrl) =>
        (destinationPath: SanitizedAbsPath): Future<TaqError, SanitizedAbsPath> =>
            pipe(
                execText('git clone <%= it.url %> <%= it.outputDir %>', {
                    url: url.value,
                    outputDir: destinationPath.value,
                }),
                mapRej<TaqError, TaqError>((previous) => ({
                    kind: 'GIT_CLONE_FAILED',
                    msg: `Could not clone ${url.value}. Please check the Git url and ensure that Git is installed.`,
    ...
    const execText = (
        ...
        bufferOutput = false,
        cwd?: SanitizedAbsPath,
    ): Future<TaqError, number | string> =>
        attemptP(async () => {
