...
const getCompileCommand = (opts: Opts) => (sourceFile: string) => {
  ...
  const baseCommand = `DOCKER_DEFAULT_PLATFORM=linux/amd64 docker run --rm -v \"${process.env['PROJECT_DIR'] || projectDir}\":/project -w /project completium/archetype:1.2.12 ${inputFile}`
