...
const getDecompileCommand = (opts: Opts) => (sourceFile: string) => {
	const { projectDir } = opts;
	const inputFile = getInputFilename(opts)(sourceFile);
	const baseCommand =
		`DOCKER_DEFAULT_PLATFORM=linux/amd64 docker run --rm -v \"${projectDir}\":/project -u $(id -u):$(id -g) -w /project ${getArchetypeDockerImage()} -d ${inputFile}`;
	const outFile = `-o ${getContractArtifactFilename(opts)(sourceFile)}`;
	const cmd = `${baseCommand} ${outFile}`;
	return cmd;
};

const decompileContract = (opts: Opts) => (sourceFile: string): Promise<{ contract: string; artifact: string }> =>
	// const sourceAbspath = join(opts.contractsDir, sourceFile)
	execCmd(getDecompileCommand(opts)(sourceFile))
		...
		.catch(err => {
			...
			return Promise.resolve({
				contract: sourceFile,
				artifact: 'Not decompiled',
			});
		});

const decompileAll = (opts: Opts): Promise<{ contract: string; artifact: string }[]> => {
	const contracts = getContracts(/\.arl$/, opts.config);
	return Promise.all(contracts)
		.then(entries => entries.map(decompileContract(opts)))
		.then(processes =>
			processes.length > 0
				? processes
...
const decompile = (parsedArgs: RequestArgs.t) => {
	...
	return match(unsafeOpts)
		...
		.otherwise(() => {
			const p = unsafeOpts.sourceFile
				? decompileContract(unsafeOpts)(unsafeOpts.sourceFile)
					.then(result => [result])
				: decompileAll(unsafeOpts)
					.then(results => {
						if (results.length === 0) sendErr('No contracts found to decompile.');
						return results;
					});
