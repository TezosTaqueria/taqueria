...
const getArtifacts = (sourceAbspath: string) => {
    return readFile(sourceAbspath, { encoding: 'utf-8' }).then((source: string) => {
        const pattern = new RegExp(/add_compilation_target\s*\(\s*(['"])([^'"]+)\1/, 'mg')
        const results = source.matchAll(pattern)
        return results ? Array.from(results).map((match) => match[2]) : ['--']
    })
}

const getCompileCommand = (opts: Opts) => (sourceAbspath: string) =>
    `~/smartpy-cli/SmartPy.sh compile ${sourceAbspath} ${opts.artifactsDir}`
...
const compileAll = (opts: Opts): Promise<{ contract: string; artifacts: string[] }[]> => {
    ...
    return glob(['**/*.py'], { cwd: opts.contractsDir, absolute: false })
...
export const compile = <T>(parsedArgs: Opts): LikeAPromise<PluginResponse, Failure<T>> => {
    const p = parsedArgs.sourceFile
        ? compileContract(parsedArgs)(parsedArgs.sourceFile as string).then((data) => [data])
        : compileAll(parsedArgs).then((results) => {
