...
const isSmartPyFile = (sourceFile: string): boolean => /.+\.py$/.test(sourceFile);

const isStorageListFile = (sourceFile: string): boolean => /.+\.(storageList|storages)\.py$/.test(sourceFile);

const isParameterListFile = (sourceFile: string): boolean => /.+\.(parameterList|parameters)\.py$/.test(sourceFile);

const isContractFile = (sourceFile: string): boolean =>
	isSmartPyFile(sourceFile) && !isStorageListFile(sourceFile) && !isParameterListFile(sourceFile);
...
const getCompileContractCmd = async (parsedArgs: Opts, sourceFile: string): Promise<string> => {
	...
	const baseCmd =
		`DOCKER_DEFAULT_PLATFORM=linux/amd64 docker run --rm -v \"${projectDir}\":/project -w /project -u $(id -u):$(id -g) ${getSmartPyDockerImage()} compile contract`;
...
const getCompileExprCmd = (parsedArgs: Opts, sourceFile: string, exprKind: ExprKind, exprName: string): string => {
	...
	const baseCmd =
		`DOCKER_DEFAULT_PLATFORM=linux/amd64 docker run --rm -v \"${projectDir}\":/project -w /project -u $(id -u):$(id -g) ${getSmartPyDockerImage()} compile ${compilerType}`;
	const inputFile = getInputFilenameRelPath(parsedArgs, sourceFile);
