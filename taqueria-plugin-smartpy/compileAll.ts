...
import {compileContract, COMPILE_ERR_MSG} from "./compile"
    ...
    try {
        ...
        const data = await allFiles.reduce(
            async (acc, contractAbsPath) => {
                ...
                if (isSmartPyFile(contractAbsPath) && !(isParameterListFile(contractAbsPath) || isStorageListFile(contractAbsPath))) {
