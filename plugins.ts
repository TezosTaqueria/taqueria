...
export const inject = (deps: PluginDeps) => {
    ...
    // logPluginRequest
    const logPluginRequest =
        (plugin: InstalledPlugin) =>
        (cmd: (string | number | boolean)[]): Future<TaqError, void> =>
            attemptP(async () => {
    ...
    const execPluginText = (cmd: string[]): Future<TaqError, string> =>
        attemptP(async () => {
            try {
    ...
    const execPluginPassthru = (cmd: string[]): Future<TaqError, Deno.Process> =>
        attemptP(async () => {
            try {
                const process = Deno.run({ cmd, stdout: 'piped', stderr: 'piped' })
    ...
    const execPluginJson = (cmd: string[]): Future<TaqError, PluginResponse> =>
        pipe(execPluginText(cmd), chain<TaqError, string, PluginResponse>(decodeJson))
    ...
    const getPluginExe = (plugin: InstalledPlugin) => {
        switch (plugin.type) {
            case 'npm': {
                const pluginPath = joinPaths(parsedArgs.projectDir.value, 'node_modules', plugin.name, 'index.js')
    ...
    const sendPluginActionRequest =
        (plugin: InstalledPlugin) =>
        (action: PluginAction) =>
        (requestArgs: Record<string, unknown>): Future<TaqError, PluginResponse> => {
            const cmd = [
                ...getPluginExe(plugin),
                '--taqRun',
    ...
    const retrievePluginInfo = (plugin: InstalledPlugin) =>
        pipe(
    ...
    const toPluginArguments = (requestArgs: Record<string, unknown>): PluginRequestArgs => {
        ...
        return Object.entries({ ...parsedArgs, ...requestArgs }).reduce(
            (retval: (string | number | boolean)[], [key, val]) => {
                ...
                else if (val instanceof SanitizedAbsPath) return [...retval, '--' + key, `'${val.value}'`]
    ...
    const writeState =
        (stateAbspath: SanitizedAbsPath) =>
        (state: State): Future<TaqError, State> =>
            pipe(
    ...
    const getStateAbspath = (): SanitizedAbsPath => parsedArgs.projectDir.join(parsedArgs.configDir.value, 'state.json')
    ...
    const getNonMemoizedState = () =>
        pipe(parsedArgs, getStateAbspath, (stateAbspath) =>
            pipe(
                !parsedArgs.disableState ? resolve(stateAbspath.value) : reject('State disabled!'),
                ...
                chain((state: State) =>
                    config.hash.value === state.configHash.value ? resolve(state) : computeState(stateAbspath),
                ),
            ),
        )
    ...
    const getMemoizedState = (() => {
        ...
        const toMemHash = (hash: string, parsedArgs: SanitizedInitArgs) => JSON.stringify({ hash, parsedArgs })
