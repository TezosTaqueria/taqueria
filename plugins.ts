...
export const inject = (deps: PluginDeps) => {
	...
	const logPluginRequest =
		(plugin: InstalledPlugin.t) => (cmd: (string | number | boolean)[]): Future<TaqError.t, void> =>
			attemptP(async () => {
				if (parsedArgs.logPluginRequests) {
					...
					// Issue that captures this: https://github.com/pinnacle-labs/taqueria/issues/732
					if (parsedArgs.debug) {
						if (/^node /.test(output)) output = output.replace(/^node /, 'node --inspect-brk ');
					}
					if (!env.get('CI') && !env.get('TEST')) await clipboard.writeText(output.replace('\\\n', ''));
				}
	...
	const toPluginArguments = (requestArgs: Record<string, unknown>): PluginRequestArgs => {
		...
		return Object.entries({ ...passableParsedArgs, ...passableRequestArgs, projectDir: config.projectDir }).reduce(
			(retval: (string | number | boolean)[], [key, val]: [string, unknown]) => {
				const omit = [
					'$0',
					'quickstart',
					'scaffoldUrl',
	...
	const writeState =
		(stateAbspath: SanitizedAbsPath.t) => (state: EphemeralState.t): Future<TaqError.t, EphemeralState.t> =>
			pipe(
