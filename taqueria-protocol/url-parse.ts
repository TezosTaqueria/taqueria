...
export const urlParse = (data: IUrlParse | string | null | undefined = {}): IUrl => {
    ...
    const { protocol = 'http', hostname, pathname, port, username, password, query = null } = innerData
    ...
    if (protocol) {
        const toUseProtocol = protocol.trim().endsWith(':') ? protocol.trim() : `${protocol.trim()}:`
