		...
		async (retval, filename) => {
			...
			if (storage === undefined || storage === null) {
				sendErr(
					...
					}.storage["${filename}]\nThe value of the above JSON key should be the name of a file, in ./${parsedArgs.config.artifactsDir}/storage/, that contains the actual value of the storage, as a Michelson expression.\n`,
				);
				return retval;
			}
