...
const originateContractToSandbox = async (
    contractFilename: string,
    parsedArgs: Opts,
    storage: unknown,
    sandboxName: string,
    sandbox: SandboxConfig | undefined,
): LikeAPromise<OriginationResult, Failure<unknown>> => {
    try {
...
const originateContractToNetwork = async (
    contractFilename: string,
    parsedArgs: Opts,
    storage: unknown,
    networkName: string,
...
const originateContract = (parsedArgs: Opts) => (contractFilename: string) => {
    ...
    const currentEnv = parsedArgs.env
        ...
        : 'development'
    const env =
        parsedArgs.config.environment && parsedArgs.config.environment[currentEnv]
    ...
    const networkProcesses = !env.networks
        ? []
        : env.networks.reduce(
              (retval: LikeAPromise<OriginationResult, Failure<unknown>>[], network) =>
                  getNetworkConfig(network, parsedArgs.config)
                      ? [
                            ...retval,
                            originateContractToNetwork(
                                contractFilename,
                                parsedArgs,
                                env.storage[contractFilename],
                                network,
                                getNetworkConfig(network, parsedArgs.config),
                            ),
                        ]
    ...
    const allProcesses = !env.sandboxes
        ? []
        : env.sandboxes.reduce(
              (retval: LikeAPromise<OriginationResult, Failure<unknown>>[], sandbox) =>
                  getSandboxConfig(sandbox, parsedArgs.config)
                      ? [
                            ...retval,
                            originateContractToSandbox(
                                contractFilename,
                                parsedArgs,
                                env.storage[contractFilename],
                                sandbox,
