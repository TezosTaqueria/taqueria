...
const attributesToParams = (attributes: Attributes): Record<string, string> =>
    ['blockTime'].reduce((retval: Record<string, string>, attributeName: string) => {
...
const getStartCommand = (sandbox: Sandbox, image: string, config: Opts, arch: string, debug: boolean): string => {
    ...
    const ports = debug ? `-p ${sandbox.rpcUrl.url.port}:20000 -p 19229:9229` : `-p ${sandbox.rpcUrl.url.port}:20000`
...
const startInstance =
    ...
    (sandbox: Sandbox): Promise<void> => {
        ...
        return isSandboxRunning(sandbox.name.value).then((running) =>
            running
...
const startAll = (sandboxes: Sandbox[], opts: Opts): Promise<void> => {
    ...
    const jobs = sandboxes.filter((sandbox) => doesUseFlextesa(sandbox)).map(startInstance(opts))
...
const getSandboxes = (parsedArgs: Opts): Sandbox[] =>
    Object.keys(parsedArgs.config.sandbox).reduce((retval: Sandbox[], sandboxName) => {
        const sandbox = getSandbox({ ...parsedArgs, sandboxName })
        return sandbox ? [...retval, sandbox] : retval
    }, [])
        ...
        if (sandbox) {
            if (doesUseFlextesa(sandbox)) {
                return (await isSandboxRunning(sandbox.name.value))
                    ? getAccountBalances(sandbox).then(sendJsonRes)
...
const stopSandboxTask = async <T>(parsedArgs: Opts): LikeAPromise<void, Failure<T>> => {
    if (parsedArgs.sandboxName) {
        ...
        if (sandbox) {
            if (doesUseFlextesa(sandbox)) {
                return (await isSandboxRunning(sandbox.name.value))
                    ? execCmd(`docker kill ${sandbox.name}`).then((_) => sendAsyncRes(`Stopped ${sandbox.name.value}.`))
