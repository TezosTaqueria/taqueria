...
export const generateTestProject = async (
    ...
    if (packageNames.length > 0) {
        packageNames.forEach((packageName) => {
            try {
                if (localPackages) {
                    const _installOut = execSync(
                        `cd ./${projectPath} && taq install ../../../taqueria-plugin-${packageName}`,
                        { encoding: 'utf8' },
                    )
                } else {
...
export function getContainerName(dockerName: string): string {
    const [_dockerContainerHeader, dockerContainerName] = execSync(`docker ps --filter "name=${dockerName}" --no-trunc`)
        .toString()
        .trim()
        .split(/\r?\n/)
