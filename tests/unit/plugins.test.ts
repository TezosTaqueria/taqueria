...
Deno.test('inject()', async (t) => {
    ...
    const config = await toPromise(toConfigArgs('config.json', configDir, projectDir.join('.taq'), defaultConfig))
    ...
    await t.step('execPluginJson() returns object with nothing output to stdout, but an error on stderr', async () => {
        ...
        const output = await toPromise(
            execPluginJson([
                'sh',
                '-c',
                "node -e \"console.log(JSON.stringify({foo:'bar'}));console.error('foobar')\"",
            ]),
        )
