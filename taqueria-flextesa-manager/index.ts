...
const parseConfig = (input: ConfigInput) => {
    ...
    const parseAccounts = (input: AccountsInput): Accounts | null =>
        Object.entries(input).reduce((retval, [accountName, accountDetailsInput]) => {
            if (typeof accountDetailsInput !== 'string') {
                ...
                return temp[accountName] ? { ...retval, ...temp } : retval
            } else {
    ...
    const parseSandboxes = (input: SandboxesInput): Sandboxes | undefined =>
        Object.entries(input).reduce((retval: Sandboxes | undefined, input: [string, SandboxSettingsInput]) => {
            ...
            if (sandboxName !== 'default') {
                ...
                return temp[sandboxName] !== undefined ? ({ ...retval, ...temp } as Sandboxes) : retval
            }
        }, {} as Sandboxes)
...
const addAccountKeys = async ([accountName, accountDetails]: [string, AccountDetails]) => {
    return [
        accountName,
        accountName === 'default' ? accountDetails : { ...accountDetails, keys: await getAccountKeys(accountName) },
    ]
}
...
const getBootstrapFlags = (sandboxName: string, config: Config) => {
    ...
    return L.collect(lens, config)
        .reduce((retval: string[], accountDetails: AccountDetailsWithKey | string) => {
...
const configureTezosClient = (config: Config) =>
    run(`tezos-client --endpoint http://localhost:20000 config update`).then(() => config)
...
const importAccounts = (sandboxName: string, config: Config) => {
    ...
    const processes = L.collect(accountLens, config).reduce(
        (retval: Promise<string>[], keys: AccountKeys) => [
            ...retval,
            retry(() =>
                isAccountImported(keys.alias).then((hasAccount) =>
                    hasAccount
                        ? Promise.resolve('success')
                        : run(
                              `tezos-client --protocol ${config.sandbox[sandboxName].protocol} import secret key ${keys.alias} ${keys.secretKey} --force | tee /tmp/import-key.log`,
                          ),
                ),
            ),
        ],
...
readJsonFile<ConfigInput>(inputArgs.config)
    ...
    .then((config: Config) => {
        if (inputArgs.configure) return configureTezosClient(config)
        else if (inputArgs.importAccounts) return importAccounts(inputArgs.sandbox, config)
        else return runMininet(inputArgs.sandbox)(config).then(() => config)
    })
    .then(() => process.exit(0))
    .catch((err: Error) => {
        console.error(err)
