...
1. `taq typecheck <inputFile>` - typechecks one or more Michelson smart contracts
...
```shell
taq typecheck <inputFile> -s sandboxName
```
...
- `taq simulate <inputFile> <input> [--storage value] [--sandboxName sandbox]`