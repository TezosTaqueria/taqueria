...
The structure of the command for the typecheck task including all optional parameters is:
```shell
taq simulate <inputFile> <inputData> -s sandboxName --storage valueOfInitialStorage --entrypoint entrypointName
```
 This command has 2 required parameters, and 3 optional parameters as shown above
...
|  --entrypoint   | entrypoint  | The annotation of the entrypoint to call   |
