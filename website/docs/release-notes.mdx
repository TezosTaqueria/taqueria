...
Taqueria v0.28.0 includes a significant change to the .taq/config.json schema:
- The new schema version is version 2 and indicated in the .taq/config.json file:
```json
{
    version: "v2",
    ...
...
#### Built-in Syntax Highlighting in VSCode Extension

The Taqueria VSCode extension now includes syntax highlighting for LIGO & Michelson. No other extension installations necessary.

#### Updated Workflow for SmartPy Developers

(Ask Jimmy to write something here)

#### Generating Test Stubs in VSCode Extension

The Taqueria CLI has had the ability to create test stubs for your LIGO contracts using our `test-contract` template. This capability has now been exposed in the VSCode Extension as well.

#### Workflows

The Taqueria CLI introduces the concept of "workflows", which allows you to specify pre-named workflows when initializing a new project:
```
taq init -w ligo`
```

These workflows will install a bundle of plugins that is typical to the workflow for a specific type of developer. Taqueria CLI ships with the following workflows:
- ligo
- smartpy
- archetype
- michelson
...
Many of our plugins, such as our Flextesa, Ligo, and Archetype plugins, use docker images to provide their respective functionality. For instance, the v0.28.0 ligo plugin uses the `ligolang/ligo:0.57.0` image underthehood.
We've added the ability to override these images using environment variables:
- TAQ_FLEXTESA_IMAGE
- TAQ_LIGO_IMAGE
- TAQ_ARCHETYPE_IMAGE

For instance, if you wanted to use LIGO v0.60 with our LIGO plugin when compiling, do the following:
```
export TAQ_LIGO_IMAGE="ligolang/ligo:0.60"
taq compile hello-tacos.mligo # will use LIGO Compiler v0.60
```

#### Deploy to Mainnet

(Ask Jimmy to fill this in)
...
In addition, you can disable baking entirely:
```
{
     "environments": {
        "development": {
...
With baking disabled, you will need to manually bake blocks by running `taq bake`. You can automatically bake blocks as new operations are injected by running the _bake_ task in _watch mode_: `taq bake -w`

#### Added `compile-all` task

Rather than compile each contract individually, you can compile all contracts of a particular type by running: `taq compile-all` for the respective plugin.


#### Added `typecheck-all` task

Like the feature above, you can now typecheck all compiled smart contracts by running `taq typecheck-all`.

#### Ability to get/set the current environment via the CLI

You can now use the `taq get-environment` task to get the current environment that will be targeted by tasks that you invoke. You can also set the environment using `taq set-environment [envName]`.

### Pull Requests

* 🚀 Release ➾ Taqueria v0.24.2 by @hu3man in https://github.com/ecadlabs/taqueria/pull/1504
* Don't log scaffold.json contents when scaffolding by @mweichert in https://github.com/ecadlabs/taqueria/pull/1545
...
* 🧽 Chore ➾ Delete legacy lerna bootstrap commands by @danielelisi in https://github.com/ecadlabs/taqueria/pull/1567
* ✨ Feature ➾ Improve SmartPy Workflow in relation to contract and storage compilations and added native SmartPy test by @jchenche in https://github.com/ecadlabs/taqueria/pull/1551
...
* 🧽 Chore ➾ Stop building native Windows binary by @danielelisi in https://github.com/ecadlabs/taqueria/pull/1601
...
* 🧪 Pre-Release ➾ RELEASE BRANCH v0.25.0 by @hu3man in https://github.com/ecadlabs/taqueria/pull/1476
* Skip problematic tests by @mweichert in https://github.com/ecadlabs/taqueria/pull/1626
...
* 🛠️ Fix ➾ Import ramda library from x.nest.land registry by @danielelisi in https://github.com/ecadlabs/taqueria/pull/1656
...
* mark the Taquito plugin  transfer tez test as flaky by @michaelkernaghan in https://github.com/ecadlabs/taqueria/pull/1695
...
* Remove contract registry from repo by @jchenche in https://github.com/ecadlabs/taqueria/pull/1807
...
## New Contributors
* @egarson made their first contribution in https://github.com/ecadlabs/taqueria/pull/1657
