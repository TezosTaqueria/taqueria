...
Add a new environment named `ithacanet`, and add the named network `ithaca` to it's `networks` array.
```json
    "environment": {
        "default": "development",
        "development": {
...
At this point, you should have a fully configured network named `ithaca` which is configured to target the ithaca testnet using a faucet, and an environment named `ithacanet` which can be used to target the network from the CLI
