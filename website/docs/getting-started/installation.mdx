...
Windows users will run the Linux version of Taqueria in Unbuntu using Windows Subsystem for Linux ([WSL 2](https://docs.microsoft.com/en-us/windows/wsl/install))
...
1. Open a Power Shell or Windows Command Prompt and run:
...
3. Open the Ubuntu distribution using the Start menu
...
2. Run the installer making sure to enable WSL 2 during the installation when prompted

3. Start Docker Desktop from the Windows Start menu

4. From the Docker menu, navigate to `Settings > General` and select `Use WSL 2 based engine`
...
To confirm that Node v18.x has been installed properly, open a new WSL 2 terminal and run:

```shell
node --version
```

You should see `v18.1.0` or similar returned
