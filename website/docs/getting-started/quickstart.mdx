...
Plugins are installed using the `taq install` command which appears in the command list once a project is initialized. You can view the list of commands available in the current project context by running `taq --help` from the CLI. It should lookk something like this:
```shell
❯ taq --help
taq <command>
