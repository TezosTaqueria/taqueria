...
Once installed, the plugin provides the task `taq compile` which when run, will look for any LIGO files in the `/contracts` directory and compile them to Michelson `.tz` files in the `/artifacts` directory. The plugin also provides a template to easily create and register a Ligo contract using the `taq create contract task
...
You need to now register the contract you want to compile with Taqueria so it knows what it can work with:
```shell
taq add-contract counter.jsligo
```

Taqueria will output that a contract has been added:
```shell
┌────────────────┬────────────────┬─────────────────┐
│ Name           │ Source File    │ Last Known Hash │
├────────────────┼────────────────┼─────────────────┤
│ counter.jsligo │ counter.jsligo │ 980f8dfb        │
└────────────────┴────────────────┴─────────────────┘
```
