---
title: Scaffolding the Quickstart Project
---
...
:::note
In a rush? The tldr of this tutorial involves the the following steps and will result in a running full-stack dapp:
...
The root Node project contains `setup` and `start` scripts which can be run together on a single line to build, deploy, and launch the dapp. This process does the following steps: 
...
If you inspect the scripts in `/taqueria/package.json`, you will see the following actions are executed when the setup command is run:
...
3. Generates TS types: `taq generate types` - uses the Taqueria Contract Types plugin to generate TS types from the compiled smart contract, and install them into the React app (`/app/types`) for use with Taquito in the frontend
