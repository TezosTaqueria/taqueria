...
- The Taqueria CLI binary has been [installed](./installation.mdx) locally and is available in your terminal `$PATH` 
...
The root Node project contains a one line script which will run the `setup` and `start` scripts for the root project. This will install the npm dependencies, run the required scripts to build and deploy the Taqueria project, then start up the React app
