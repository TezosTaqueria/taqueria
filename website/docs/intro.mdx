...
- Originating a smart contract to a sandbox and testnet with the [Taquito](/docs/plugins/plugin-taquito/) plugin
- Generating TypeScript types from a Michelson contract with the [Contract Types](/docs/plugins/plugin-contract-types/) plugin
- Typechecking Michelson contracts and simulating contract calls with the [Octez Client](/docs/plugins/plugin-octez-client/) plugin
...
Taqueria plugins are installed on a ***per-project*** basis which allows you to customize the combination of technologies used and tasks available on a project. This means that the tasks available to run in each project will differ based on the plugins installed
