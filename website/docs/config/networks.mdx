...
```
:::note
The information for `rpcUrl` and`protocol` are taken from the [https://teztnets.xyz/ghostnet-about](https://teztnets.xyz/ghostnet-about) page
:::
...
Add a new environment named `testing`, and add the named network `ghost` to its `networks` array:
```json
    "environment": {
        "default": "development",
        "development": {
...
At this point, you should have a fully configured network named `ghostnet` which is configured to target [ghostnet](https://teztnets.xyz/ghostnet-about) using a faucet, and an environment named `ghostnet` which can be used to target the network from the CLI
...
Taqueria uses [Environments](/docs/configuration/environments) to target a specific network or sandbox. An environment is a named collection of network configurations which can be passed to the CLI using the `--env` flag

To target the `ghost` network we configured above using the `ghostnet` environment, simply run the following command:
```shell
