...
The account balance represents the target balance for those accounts. The enviroment plugins will provide a mechnism to instantiate and maintain the balance for those account.
...
Since this is specific to the developer's sandbox, these accounts are in the `config.local...` file for the sandbox environemnt.
...
For other enviroment types, the taquito plugin has the `instantiate` and `fund` tasks to manually maintain these accounts.
