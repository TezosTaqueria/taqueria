...
Each environment has a type. The `development` enviroment is `flextesa` which uses the flextesa plugin for a local sandbox. The `testing` and `production` environments are both the `simple` type which use only the rpcUrl. 

Other enviroment types can be created with a plugin and it can define additional enviroment fields as needed.

## The Default Environment

In the above example, the default environemnt is `development`. For most taq commands the default enviroment will be used unless the `--env` flag is provided.

## Local Config

Each environment also has a `config.local...` file. 

Going back to the example `config.json` above, it has three enviroments. So, each would have their own `config.local...` file:
