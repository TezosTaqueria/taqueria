...
:::note
These sandbox configurations are used by sandbox plugins. At this time, Taqueria supports the Flextesa sandbox with the [Taqueria Flextesa Plugin](/docs/plugins/flextesa)
:::
...
Inside the sandboxConfigObject, there are three static properties you can configure, and the `accounts` property which is automatically populated when you start up a sandbox
...
A string which corresponds to the local URL you would like the sandbox to run on. The default is `http://localhost:2000`
