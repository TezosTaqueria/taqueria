...
| Repository         | https://github.com/tezostaqueria/taqueria-scaffold-taco-shop |
...
1. `taq scaffold https://github.com/tezostaqueria/taqueria-scaffold-taco-shop taco-shop`
1. `cd taco-shop`
1. `npm run setup`
1. `npm run start:app`
...
The smart contract has been deployed to shadownet at the address for demonstration purposes: `KT1KBBk3PXkKmGZn3K6FkktqyPRpEbzJoEPE`
...
- A network configuration for the Shadownet testnet
- An environment named `shadownet` with faucet to fund operations on the testnet
...
```shell
taq scaffold https://github.com/tezostaqueria/taqueria-scaffold-taco-shop taco-shop
```
...
Run the following command to originate the contract to the shadownet environment:
...
├────────────────┼──────────────────────────────────────┼────────────────┤────────────────┤
│ hello-tacos.tz │ KT1KBBk3PXkKmGZn3K6FkktqyPRpEbzJoEPE │ hello-tacos    │ shadownet       │
└────────────────┴──────────────────────────────────────┴────────────────┘────────────────┘
```
...
function App() {
  ...
  const [rpcUrl] = useState("https://tezos-shadownet.octez.io");
...
This scaffold comes with Jest tests in the `tests` folder which has been initialized as a partition. The scaffold uses the `@taqueria/plugin-jest` plugin to run the tests
