...
function Feature({ title, description, features, button }) {
    const checkTitle = (e) => {
        setUserAction(true)
        isVisible === e.target.id ? toggleIsVisible(`${features[0].title}`) : toggleIsVisible(e.target.id)
    }
        ...
        <div>
            ...
            <div className={styles.featuresBlock}>
                ...
                <div className={styles.videoBlock}>
                    ...
                    {features.map((feature, index) => {
                        if ('img' in feature) {
                            return isVisible === feature.title && <img id={feature.title} src={feature.img} />
                        } else {
                            return (
                                isVisible === feature.title && (
                    ...
                    {features.map((feature, index) => {
                        return (
                            <div key={index} className={styles.featuresContent}>
                                <div className={styles.featureTabs} id={feature.title} onClick={(e) => checkTitle(e)}>
                                    <img id={feature.title} className={styles.briefCase} src={feature.icon} />
                                    <h4 className={styles.featureTitle} id={feature.title}>
                                        {feature.title}
                                    </h4>
                                </div>
