...
export const displayError = (cli:CLIConfig) => (err: Error|TaqError) => {
    ...
    if (!inputArgs.help) {
        const res = match(err)
            .with({kind: 'E_FORK'}, (err: TaqError)                         => [125, err.msg])
            .with({kind: 'E_INVALID_CONFIG'}, (err: TaqError)               => [1, err.msg])
            .with({kind: 'E_INVALID_PATH_ALREADY_EXISTS'}, (err: TaqError)  => [3, `${err.msg}: ${err.context}`])
            .with({kind: 'E_INVALID_PATH_DOES_NOT_EXIST'}, (err: TaqError)  => [4, `${err.msg}: ${err.context}`])
            .with({kind: 'E_INVALID_TASK'}, (err: TaqError)                 => [5, err.msg])
            .with({kind: 'E_INVALID_PLUGIN_RESPONSE'}, (err: TaqError)      => [6, err.msg])
            .with({kind: 'E_MKDIR_FAILED'}, (err: TaqError)                 => [7, `${err.msg}: ${err.context}`])
            .with({kind: 'E_NPM_INIT'}, (err: TaqError)                     => [8, err.msg])
            .with({kind: 'E_READFILE'}, (err: TaqError)                     => [9, err.msg])
            .with({kind: 'GIT_CLONE_FAILED'}, (err: TaqError)               => [10, err.msg])
            .with({kind: 'E_INVALID_ARGS'}, (err: TaqError)                 => [11, err.msg])
            .with({kind: 'E_INVALID_JSON'}, (err: TaqError)                 => [12, err])
            .with({kind: 'E_PLUGIN_NAME_REQUIRED'}, (err: TaqError)         => [13, err.msg])
            .with({message: __.string}, (err: Error)                     => [128, err.message])
            .exhaustive()
