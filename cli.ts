...
import {
    ...
    parallel,
    debugMode,
} from 'https://cdn.jsdelivr.net/gh/fluture-js/Fluture@14.0.0/dist/module.js'
...
import {
    ...
    TaqError,
    Future,
} from './taqueria-utils/taqueria-utils-types.ts'
...
) => (action: PluginAction) => (requestArgs: Record<string, unknown>) => Future<TaqError, PluginResponse>
...
const getVersion = (inputArgs: DenoArgs, i18n: i18n) => {
    ...
    return i > -1 ? inputArgs[i + 1] : 'not-provided'
}
...
const commonCLI = (env: EnvVars, args: DenoArgs, i18n: i18n) =>
    yargs(args)
        ...
        .command(
            ...
            (args: RawInitArgs) =>
                pipe(sanitizeArgs(args), scaffoldProject(i18n), forkCatch(console.error)(console.error)(console.log)),
        )
        .option('fromVsCode', {
...
const initCLI = (env: EnvVars, args: DenoArgs, i18n: i18n) => pipe(commonCLI(env, args, i18n).help(false))

const postInitCLI = (cliConfig: CLIConfig, env: EnvVars, args: DenoArgs, parsedArgs: SanitizedInitArgs, i18n: i18n) =>
    pipe(
...
const parseArgs = (cliConfig: CLIConfig): Future<TaqError, RawInitArgs> =>
    pipe(
        attemptP<Error, RawInitArgs>(() =>
            cliConfig.parseAsync().then((rawInitArgs: unknown) => rawInitArgs as RawInitArgs),
        ),
...
const listKnownTasks = (cliConfig: CLIConfig, parsedArgs: SanitizedInitArgs) => (_inputArgs: RawInitArgs) =>
    pipe(
        ...
        map((state) =>
            Object.entries(state.tasks).reduce((retval, [taskName, implementation]) => {
                if ('task' in implementation) {
                    const task = implementation as Task
                    const plugins = task.options
                        ? task.options.reduce(
                              (retval: string[], option) => (option.choices ? [...retval, ...option.choices] : retval),
                              [],
                          )
                        : []
...
const initProject = (
    ...
    pipe(
        ...
        chain(({ artifactsDir, contractsDir, testsDir, projectDir }: ConfigArgs) => {
            const artifactsAbspath = joinPaths(projectDir.value, artifactsDir),
...
const scaffoldProject =
    (i18n: i18n) =>
    ({ scaffoldUrl, scaffoldProjectDir }: SanitizedInitArgs) =>
        pipe(
            ...
            bichain<TaqError, TaqError, SanitizedAbsPath>((err) =>
                err.kind === 'E_INVALID_PATH_DOES_NOT_EXIST' ? resolve(scaffoldProjectDir) : reject(err),
            )(resolve),
...
const getCanonicalTask = (pluginName: string, taskName: string, state: State) =>
...
const loadState = (
    ...
    config: ConfigArgs,
    env: EnvVars,
    parsedArgs: SanitizedInitArgs,
    i18n: i18n,
    state: State,
    sendPluginActionRequest: SendPluginActionRequest,
): CLIConfig =>
    Object.entries(state.tasks).reduce((retval: CLIConfig, pair: [string, InstalledPlugin | Task]) => {
        ...
        return foundTask
            ? addTask(retval, config, env, parsedArgs, i18n, foundTask, sendPluginActionRequest, implementation)
            : retval
    }, cliConfig)
    ...
    pipe(
        cliConfig.command({
            ...
            builder: (cliConfig: CLIConfig) => {
                if (task.options)
                    task.options.reduce((cli: CLIConfig, option: Option) => {
                ...
                if (task.positionals)
                    task.positionals.reduce((cli: CLIConfig, positional: PositionalArg) => {
                    ...
                    task.handler === 'proxy' && plugin
                        ...
                        : pipe(
                              exec(
                                  ...
                                  ['json', 'application/json'].includes(task.encoding),
                              ),
...
const renderTable = (data: Record<string, string>[]) => {
    const keys: string[] = pipe(
        data.reduce((retval: string[], record) => [...retval, ...Object.keys(record)], []),
        uniq,
    )

    const rows = data.reduce((retval: string[][], record) => {
    ...
    !parsedArgs.plugin
        ...
        : {
              ...
              plugin: state.plugins.reduce(
                  (retval, pluginInfo: PluginInfo) => (pluginInfo.alias === retval ? pluginInfo.name : retval),
                  parsedArgs.plugin,
              ),
          }

const extendCLI = (env: EnvVars, parsedArgs: SanitizedInitArgs, i18n: i18n) => (cliConfig: CLIConfig) =>
    pipe(
        ...
        chain((config: ConfigArgs) => {
            ...
            return pipe(
                pluginLib.getState(),
                map((state: State) =>
                    pipe(resolvePluginName(parsedArgs, state), (parsedArgs: SanitizedInitArgs) =>
                        loadState(cliConfig, config, env, parsedArgs, i18n, state, pluginLib.sendPluginActionRequest),
                    ),
                ),
            )
        }),
...
const executingBuiltInTask = (inputArgs: SanitizedInitArgs | RawInitArgs) =>
    ['init', 'install', 'uninstall', 'testFromVsCode', 'list-known-tasks', 'listKnownTasks'].reduce(
...
export const run = (env: EnvVars, inputArgs: DenoArgs, i18n: i18n) => {
    try {
        ...
        return pipe(initCLI(env, inputArgs, i18n), (cliConfig: CLIConfig) =>
            pipe(
                cliConfig,
                parseArgs,
                map(sanitizeArgs),
...
export const displayError = (cli: CLIConfig) => (err: Error | TaqError) => {
    ...
    if (!inputArgs.fromVsCode) {
        cli.getInternalMethods()
            .getUsageInstance()
            .showHelp(inputArgs.help ? 'log' : 'error')
    }
