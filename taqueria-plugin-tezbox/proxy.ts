...
import type { Opts, TezBoxAnnotations, ValidLoadedConfig, ValidOpts } from './types';
...
export const getContainerName = async (parsedArgs: ValidOpts) => {
	...
	return `taq-tezbox-${uniqueSandboxName}`;
};
...
const getTezBoxAnnotations = (sandbox: SandboxConfig.t): Promise<TezBoxAnnotations> => {
	const defaults = {
		baking: 'enabled',
		block_time: 1,
	};
	...
	if (!['enabled', 'disabled'].includes(settings.baking)) {
		return Promise.reject(
			'The "baking" setting of a TezBox Sandbox must to set to either "enabled" or "disabled".',
		);
	} else if (!Number.isInteger(settings.block_time)) {
		return Promise.reject(
			'The "block_time" setting of a TezBox Sandbox must be an integer, and set to a value greater than 0.',
		);
	} else if (settings.block_time <= 0) {
		return Promise.reject(
			'The "block_time" setting of a TezBox Sandbox must be set to a value greater than 0. If you are trying to disable baking, please set the "baking" setting to "disabled" instead.',
		);
	}

	return Promise.resolve(settings as TezBoxAnnotations);
};

const getBakingFlags = (sandbox: SandboxConfig.t) =>
	getTezBoxAnnotations(sandbox)
		.then(settings => {
			// Enabled
...
const doesUseTezBox = (sandbox: SandboxConfig.t) => !sandbox.plugin || sandbox.plugin === 'tezbox';

const doesNotUseTezBox = (sandbox: SandboxConfig.t) => !doesUseTezBox(sandbox);
...
const taskMap: Record<string, (opts: ValidOpts) => Promise<void>> = {
	...
	'start tezbox': startSandboxTask,
	'stop sandbox': stopSandboxTask,
	'stop tezbox': stopSandboxTask,
	'bake': bakeTask,
	'b': bakeTask,
	'restart sandbox': restartSandboxTask,
	'restart tezbox': restartSandboxTask,
};
...
const validateRequest = async (unparsedArgs: Opts) => {
	...
	if (!unparsedArgs.task || !Object.keys(taskMap).includes(unparsedArgs.task)) {
		return await sendAsyncErr(`${unparsedArgs.task} is not an understood task by the TezBox plugin`);
	}
