...
const getCompileCommand = (opts: Opts) => (sourceFile: string) => {
    ...
    const baseCommand = `DOCKER_DEFAULT_PLATFORM=linux/amd64 docker run --rm -v \"${projectDir}\":/project  -v "$PROJECT_DIR:$PROJECT_DIR" -w /project ligolang/ligo:next compile contract ${inputFile}`
