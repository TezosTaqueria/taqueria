import {
    ...
    PositionalArg as aPositionalArg,
    Alias,
    Option as anOption,
    Network as aNetwork,
    UnvalidatedOption as OptionView,
    Task as TaskLike,
    EconomicalProtocol as anEconomicalProtocol,
    PluginResponse,
...
export const writeJsonFile =
    ...
    (data: T): Promise<string> =>
        writeFile(filename, JSON.stringify(data, undefined, 4), { encoding: 'utf8' }).then((_) => filename)
...
export const parseJSON = (input: string): LikeAPromise<Config, Failure<string>> =>
    new Promise((resolve, reject) => {
        try {
...
export const sendRes = (msg: string, newline = true) => {
    ...
    return newline ? console.log(msg) : (process.stdout.write(msg) as unknown as void)
}
...
export const sendErr = (msg: string, newline = true) => {
    if (!msg || msg.length === 0) return
    return newline ? console.error(msg) : (process.stderr.write(msg) as unknown as void)
}
...
const parseConfig = (config: string | Record<string, unknown>): Promise<Record<string, unknown>> =>
    typeof config === 'string' ? parseJSON(config) : Promise.resolve(config)
...
const viewTask = ({
    ...
    options: !options
        ? []
        : options.reduce(
              (retval: OptionView[], option: anOption | undefined) =>
                  option ? [...retval, viewOption(option)] : retval,
              [],
          ),
    positionals: !positionals
        ? []
        : positionals.reduce(
              (retval: PositionalArgView[], arg: aPositionalArg | undefined) =>
                  arg ? [...retval, viewPositionalArg(arg)] : retval,
              [],
          ),
...
const parseSchema = (i18n: i18n, definer: pluginDefiner, inferPluginName: () => string): SchemaView | undefined => {
    try {
        const { name, alias, schema, version, tasks, scaffolds, hooks, networks, sandboxes, ...functions } =
            definer(i18n)
        ...
        return {
            ...
            version,
            tasks: tasks
                ? tasks.reduce((retval: TaskView[], task) => (task ? [...retval, viewTask(task)] : retval), [])
                : [],
            hooks: [],
            scaffolds: [],
            networks: [],
...
const getResponse =
    (definer: pluginDefiner, inferPluginName: () => string) =>
    (sanitzedArgs: SanitizedArgs): LikeAPromise<PluginResponse, Failure<[]>> => {
...
const inferPluginName = (stack: ReturnType<typeof get>): (() => string) => {
    ...
    const pluginManifest = stack.reduce((retval: null | string, callsite) => {
    ...
    return () => (!pluginManifest ? generateName().dashed : getNameFromPluginManifest(pluginManifest))
}
