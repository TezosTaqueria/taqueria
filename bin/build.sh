...
if [ "$0" == "./bin/build.sh" -a -f index.ts ]; then
    ...
    if [  -z "$HAS_DENO" ]; then
        ...
        deno compile -o taq --unstable --allow-run --allow-write --allow-read --allow-env --allow-net --no-prompt index.ts $TARGET_ARG --setBuild "$TAQ_BUILD" --setVersion "$TAQ_VERSION"  
    fi
else
