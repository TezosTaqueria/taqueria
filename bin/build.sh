...
if [ "$0" == "./bin/build.sh" -a -f index.ts ]; then
    ...
    if [  -z "$HAS_DENO" ]; then
        ...
        DENO_DIR=./deno deno compile -o taq --allow-run --allow-write --allow-read --allow-env --import-map ./import_map.json index.ts --quickstart "`cat quickstart.md`" --setBuild "$BUILD" --setVersion "$TAQ_VERSION"
    fi
else
