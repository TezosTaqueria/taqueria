...
if [ "$0" == "./bin/run.sh" -a -f index.ts ]; then
    deno run --allow-run --unstable --allow-write --allow-read --allow-env --allow-net --import-map "${PROJ_DIR}/import_map.json" index.ts --setBuild "$BUILD" --setVersion "$TAQ_VERSION"  --lock ./deno-lock.json $@
else
