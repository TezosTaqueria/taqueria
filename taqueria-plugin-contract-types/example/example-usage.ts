...
import {
    ExampleContract1ContractType as TestContract,
    ExampleContract1WalletType as TestWallet,
} from './types-file/example-contract-1.types'
import {
    ExampleContract2ContractType as TestContract2,
    ExampleContract2WalletType as TestWallet2,
} from './types-file/example-contract-2.types'
...
export const exampleTypedOriginateStorageAndResultContract_contract = async () => {
    ...
    const originationResult = await Tezos.contract.originate<TestContract2>({
        ...
        storage: {
            assets: {
                ledger: tas.bigMap([{ key: { 0: tas.address('tz123'), 1: tas.nat(42) }, value: tas.nat(42) }]),
...
export const exampleTypedOriginateStorageAndResultContract_wallet = async () => {
    ...
    const originationResult = await Tezos.wallet
        .originate<TestWallet2>({
            code: ``,
            // Require typed initial storage
            storage: {
                assets: {
...
export const exampleBatchOrigination_Contract = async () => {
    ...
    const batchOperation = Tezos.contract
        .batch()
        // Untyped
        .withOrigination({
            code: ``,
...
export const exampleBatchOrigination_Wallet = async () => {
    ...
    const batchOperation = Tezos.wallet
        .batch()
        // Untyped
        .withOrigination({
            code: ``,
