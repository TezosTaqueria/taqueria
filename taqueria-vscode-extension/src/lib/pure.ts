        ...
        return makeDir(join(dir, '.taq'), i18n)
            .then((dotTaqDir) =>
                makeFile(join(dotTaqDir, 'config.json'), i18n)
                    ...
                    .then((pathToConfig) =>
                        readJsonFile<Config>(i18n, (data) => data as unknown as Config)(pathToConfig),
                    )
                    .catch((previous) =>
                        Promise.reject({
                            code: 'E_NOT_TAQIFIED',
 ...
 */
export const makePathToTaq =
    (i18n: I18N) =>
    (inputPath: string): PromiseLike<E_TAQ_NOT_FOUND, PathToTaq> =>
        stat(inputPath)
 ...
 */
export const execCmd = (cmd: string): PromiseLike<E_EXEC, string> =>
    new Promise((resolve, reject) => {
        if (isWindoze())
            reject({ code: 'E_WINDOWS', msg: 'Running in Windows without WSLv2 is currently not supported.' })
        else
 ...
 */
export const proxyToTaq =
    (pathToTaq: PathToTaq, i18n: I18N, projectDir?: PathToDir) =>
    (taskWithArgs: string): PromiseLike<E_PROXY, string> =>
        (projectDir
            ...
            if ('code' in previous) {
                if (previous.code === 'E_EXEC') {
                    ...
                    return decodeJson(msg)
                        .catch((_) => (msg.includes('EBADENGINE') ? 'Please install NodeJS v16.' : msg))
                        .then((err) => {
...
export const readJsonFile =
    <T>(_i18n: I18N, make: (data: Record<string, unknown>) => T) =>
    (pathToFile: PathToFile) =>
        readFile(pathToFile, { encoding: 'utf-8' }).then((data) => {
            try {
...
export const findTaqBinary = (i18n: I18N): PromiseLike<E_TAQ_NOT_FOUND, string> =>
    execCmd('which taq')
        .then((path) => path.trim())
        .catch((previous) =>
            Promise.reject({ code: 'E_TAQ_NOT_FOUND', msg: 'Could not find taq in your path.', previous }),
        )
