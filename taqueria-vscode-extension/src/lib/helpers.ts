...
export const sanitizeDeps = (deps?: InjectedDependencies) => deps ? deps : { vscode: api };
...
export class VsCodeHelper {
	...
	static async construct(
		...
		logHelper.showLog(
			OutputLevels.info,
			'the activate function was called for the Taqueria VsCode Extension.',
		);
	...
	async setup(): Promise<void> {
		...
		this.exposeTaqTaskAsCommand('opt_in', 'opt-in', 'output', {
			finishedTitle: `opted in to analytics`,
			progressTitle: `opting in to analytics`,
		});
		this.exposeTaqTaskAsCommand('opt_out', 'opt-out', 'output', {
		...
		this.exposeTaqTaskAsCommand('generate_types', 'generate types', 'output', {
		...
		// Sandbox tasks
		this.exposeSandboxTaskAsCommand('start_sandbox', 'start sandbox', {
			finishedTitle: `started sandbox`,
			progressTitle: `starting sandbox`,
		});
		this.exposeSandboxTaskAsCommand('stop_sandbox', 'stop sandbox', {
			finishedTitle: `stopped sandbox`,
			progressTitle: `stopping sandbox`,
		});
		this.exposeSandboxTaskAsCommand('list_accounts', 'list accounts', {
		...
		this.exposeGenerateTestStubTask(
			'generate_test_stub_current_file',
			'currentFile',
		);
		this.exposeGenerateTestStubTask(
			'generate_test_stub_pick_file',
			'openDialog',
		);
	...
	exposeScaffoldTask() {
		this.registerCommand(
			Commands.scaffold,
			async (scaffold?: ScaffoldTreeItem | undefined) => {
				...
				if (!scaffoldUrl) {
					...
					scaffoldUrl = availableScaffolds.find(
						template => template.name === scaffoldName,
					)?.url;
					...
					try {
						...
						if (hasInstalled) {
							this.vscode.window.showInformationMessage(
								`Successfully installed Taq CLI.`,
							);
						} else {
	...
	private formatObjectOrArrayAsTable(data: unknown) {
		...
		const rows = array.reduce((retval: string[][], record) => {
			const row = keys.reduce((row: string[], key: string) => {
	...
	async getTaqifiedDirectories() {
		const taqified = await filterAsync(
			[...this.getFolders()],
			async folder => {
				try {
	...
	async getFolderForInitOrScaffold(taskTitle: string) {
	...
	async promptForPluginInstallation(projectDir: Util.PathToDir) {
		...
		const availablePluginsNotInstalled = config.config?.plugins
			? availablePlugins.filter(
				name => config.config.plugins?.findIndex(p => p.name === name) === -1,
			)
			: availablePlugins;
		const pluginName = await this.vscode.window.showQuickPick(
			availablePluginsNotInstalled,
			{
				canPickMany: false,
	...
	async promptForPluginUninstall(projectDir: Util.PathToDir) {
		const config = await Util.TaqifiedDir.create(projectDir, this.i18);
		if (
			!config.config
			|| !config.config.plugins
			|| !config.config.plugins.length
		) {
			return undefined;
		}
		const pluginNames = config.config.plugins.map(plugin => plugin.name);
		const selectedPluginName = await this.vscode.window.showQuickPick(
			pluginNames,
			{
				canPickMany: false,
				ignoreFocusOut: false,
	...
	async getAvailablePlugins() {
		try {
			...
			if (HOME_DIR) {
				...
				const contents = await readFile(availablePluginsFile, {
					encoding: 'utf-8',
				});
	...
	async exposeInstallTask() {
		this.registerCommand(
			Commands.install,
			async (pluginInfo?: PluginTreeItem | undefined) => {
	...
	async getTaqBinPath() {
		let providedPath = this.vscode.workspace
			.getConfiguration('taqueria')
			.get('path', '');
	...
	async notifyAndLogError(
		message: string,
		err: TaqVsxError | TaqError | Error | unknown,
	) {
	...
	logAllNestedErrors(
		err: TaqVsxError | TaqError | Error | unknown,
		suppressWindow?: boolean | undefined,
	) {
		const outputLevel = suppressWindow
			? OutputLevels.debug
			: OutputLevels.error;
		if (!err) {
			return;
		...
		try {
			...
			try {
				this.logHelper.logChannel.appendLine(
					`unknown error occurred while trying to log an error.`,
				);
	...
	exposeTypecheckCommand() {
		this.registerCommand(
			'typecheck',
			async (arg: SandboxTreeItem | ContractTreeItem | undefined) => {
				const { config, pathToDir } = this.observableConfig.currentConfig;
				if (!config || !pathToDir) {
					return;
				}
			...
			async progress => {
				...
				try {
					...
					if (result.executionError || result.standardError) {
						...
						this.vscode.window.showInformationMessage(
							`Successfully ${taskTitles.finishedTitle}.`,
						);
					}
				} catch (e: unknown) {
					this.vscode.window.showErrorMessage(
	...
	async proxyToTaq(taskWithArgs: string, projectDir?: string | undefined) {
		const pathToTaq = await this.getTaqBinPath();
		const pathToDir = projectDir
			? await Util.makeDir(projectDir, this.i18)
			: undefined;
		if (projectDir) {
			...
			return await Util.execCmd(
				`${pathToTaq} --fromVsCode ${taskWithArgs}`,
				this.logHelper.showLog,
			);
		}
	}
	...
	exposeTaqTaskAsCommandWithFileArgument(
		...
		this.registerCommand(cmdId, async (item: HasFileName) => {
			await this.proxyToTaqAndShowOutput(
	...
	exposeCompileCommand(
		...
		this.registerCommand(
			...
			async (item?: HasFileName | api.Uri | undefined) => {
				...
				this.logHelper.showLog(
					OutputLevels.debug,
					`Running command ${cmdId} for ${fileName}`,
				);
	...
	async getFileNameForOperationsOnContracts(
		...
		const expectedContractsOrArtifactsFolder = path.join(
			config.dir,
			folderRelativePath,
		);

		if (
			fileSelectionBehavior === 'getFromCommand'
			|| fileSelectionBehavior === 'getFromCommandOrOpenDialog'
		) {
			if (instanceOfHasFileName(item)) {
				return item.fileName;
			} else if (item instanceof api.Uri) {
				const fileName = path.relative(
					expectedContractsOrArtifactsFolder,
					item.path,
				);
				if (!fileName) {
					this.logHelper.showLog(
		...
		if (
			fileSelectionBehavior === 'openDialog'
			|| fileSelectionBehavior === 'getFromCommandOrOpenDialog'
		) {
	...
	private async getRelativeFilePath(
		uri: api.Uri,
		folder: 'contracts' | 'artifacts',
	) {
		...
		if (!mainFolder) {
			this.logHelper.showLog(
				OutputLevels.warn,
				`No workspace is open, canceling command.`,
			);
			return undefined;
		}
		const config = await Util.TaqifiedDir.create(
			mainFolder.fsPath as Util.PathToDir,
			this.i18,
		);
	...
	exposeTaqTaskAsCommand(
		...
		this.registerCommand(cmdId, async () => {
			await this.proxyToTaqAndShowOutput(
				taskWithArgs,
				taskTitles,
				undefined,
	...
	exposeTestSetupCommand() {
		this.registerCommand('create_test_folder', async () => {
			const mainFolder = this.getMainWorkspaceFolder();
			if (!mainFolder) {
				return;
			}
	...
	exposeRunTestCommand() {
		this.registerCommand('run_tests', async (item: TestTreeItem) => {
	...
	async exposeSandboxTaskAsCommand(
		...
		this.registerCommand(
			cmdId,
			async (sandbox?: SandboxTreeItem | undefined) => {
				...
				if (!sandboxName) {
					...
					const taqifiedDir = await Util.TaqifiedDir.create(
						mainFolder.fsPath as Util.PathToDir,
						this.i18,
					);
	...
	async updateCommandStates() {
		...
		const systemCheckPassed = isTaqReachable
			&& nodeFound
			&& isNodeVersionValid
			&& nodeMeetsVersionRequirement;

		this.vscode.commands.executeCommand(
			'setContext',
			'@taqueria-state/is-taq-cli-reachable',
			isTaqReachable,
		);
		this.vscode.commands.executeCommand(
			'setContext',
			'@taqueria-state/is-node-installed',
			nodeFound,
		);
		this.vscode.commands.executeCommand(
			'setContext',
		...
		this.vscode.commands.executeCommand(
			'setContext',
			'@taqueria-state/system-check-passed',
			systemCheckPassed,
		);
		...
		if (systemCheckPassed) {
			try {
				await this.vscode.commands.executeCommand(
					'workbench.actions.treeView.taqueria-system-check.collapseAll',
				);
			} catch {
				// ignored;
			}
		...
		if (mainFolder === undefined) {
			this.logHelper.showLog(
				OutputLevels.trace,
				'No folder is open, enabling init and scaffold',
			);
			this.vscode.commands.executeCommand(
				'setContext',
		...
		this.logHelper.showLog(
			OutputLevels.trace,
			'Project config changed, updating command states...',
		);
		...
		try {
			config = await Util.TaqifiedDir.create(
				mainFolder.fsPath as Util.PathToDir,
				this.i18,
			);
			...
			if (
				shouldOutput(OutputLevels.info, this.logHelper.logLevel)
				|| taqFolderFound
			) {
				this.vscode.commands.executeCommand(
					'setContext',
					'@taqueria-state/enable-all-commands',
					true,
				);
				this.logHelper.showLog(
					OutputLevels.error,
					'Error: Could not update command states:',
				);
				this.logAllNestedErrors(e);
				if (taqFolderFound) {
					this.logHelper.showLog(
		...
		this.logHelper.showLog(
			OutputLevels.trace,
			`@taqueria-state/is-taqified: ${!!config?.config}`,
		);
		this.vscode.commands.executeCommand(
			'setContext',
			'@taqueria-state/is-taqified',
			!!config?.config,
		);
		...
		this.logHelper.showLog(
			OutputLevels.trace,
			`Known plugins: ${JSON.stringify(plugins)}`,
		);
		for (const plugin of plugins) {
			const found = config?.config?.plugins?.find(item => item.name === plugin)
				!== undefined;
			this.logHelper.showLog(OutputLevels.trace, `plugins ${plugin}: ${found}`);
			this.vscode.commands.executeCommand(
				'setContext',
				plugin,
				enableAllCommands || found,
			);
		}
		const installedCompilers = this.getCompilerPlugins()
			.filter(
				compiler =>
					config?.config?.plugins?.find(
						plugin => plugin.name === compiler,
					) !== undefined,
			)
			.map(pluginName => pluginName.replace('@taqueria/plugin-', ''));
		this.vscode.commands.executeCommand(
			'setContext',
			'@taqueria/plugin-any-compiler',
			installedCompilers.length !== 0,
		);
		...
		this.logHelper.showLog(
			OutputLevels.debug,
			`Supported extensions: ${JSON.stringify(supportedFileExtensions)}`,
		);
	}

	public static isPluginInstalled(
		config: Util.TaqifiedDir | null | undefined,
		pluginName: string,
	) {
		return (
			(config?.config?.plugins?.findIndex(p => p.name === pluginName)
				?? -1) !== -1
		);
	}
	...
	createWatcherIfNotExists(projectDir: string) {
		this.logHelper.showLog(
			OutputLevels.debug,
			`Directory ${projectDir} should be watched.`,
		);
		this.watchers.addConfigWatcherIfNotExists(projectDir, () => {
			this.logHelper.showLog(
				OutputLevels.info,
				`Adding watchers for directory ${projectDir}.`,
			);
			try {
				const taqFolderWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, '.taq'),
				);
				this._taqFolderWatcher = taqFolderWatcher;
				const configWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, '.taq/config*.json'),
				);
				this._configWatcher = configWatcher;
				const stateWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, '.taq/state.json'),
				);
				this._stateWatcher = stateWatcher;

				const contractsFolderWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, 'contracts'),
				);
				const contractsWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, 'contracts/*'),
				);
				const testsWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, '**/jest.config.js'),
				);

				const artifactsFolderWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, 'artifacts'),
				);
				const artifactsWatcher = this.vscode.workspace.createFileSystemWatcher(
					join(projectDir, 'artifacts/**/*.tz'),
				);
				...
				contractsFolderWatcher.onDidChange(
					_ => this.contractsDataProvider?.refresh(),
				);
				contractsFolderWatcher.onDidCreate(
					_ => this.contractsDataProvider?.refresh(),
				);
				contractsFolderWatcher.onDidDelete(
					_ => this.contractsDataProvider?.refresh(),
				);
				contractsWatcher.onDidChange(
					_ => this.contractsDataProvider?.refresh(),
				);
				contractsWatcher.onDidCreate(
					_ => this.contractsDataProvider?.refresh(),
				);
				contractsWatcher.onDidDelete(
					_ => this.contractsDataProvider?.refresh(),
				);

				artifactsFolderWatcher.onDidChange(
					_ => this.artifactsDataProvider?.refresh(),
				);
				artifactsFolderWatcher.onDidCreate(
					_ => this.artifactsDataProvider?.refresh(),
				);
				artifactsFolderWatcher.onDidDelete(
					_ => this.artifactsDataProvider?.refresh(),
				);
				artifactsWatcher.onDidChange(
					_ => this.artifactsDataProvider?.refresh(),
				);
				artifactsWatcher.onDidCreate(
					_ => this.artifactsDataProvider?.refresh(),
				);
				artifactsWatcher.onDidDelete(
					_ => this.artifactsDataProvider?.refresh(),
				);
				...
				throw {
					kind: 'E_UnknownError',
					msg: `Unexpected error occurred while trying to watch ${
						join(
							projectDir,
							'.taq/config.json',
						)
	...
	async registerDataProviders() {
		...
		this.registerDataProvider(
			'taqueria-environments',
			new EnvironmentsDataProvider(this),
		);
		this.testDataProvider = this.registerDataProvider(
			'taqueria-tests',
			new TestDataProvider(this),
		);
		...
		this.registerDataProvider(
			'taqueria-scaffold',
			new ScaffoldsDataProvider(this),
		);
	...
	async getTaqVersion(): Promise<string | undefined> {
		try {
			const pathToTaq = await this.getTaqBinPath();
			return await Util.checkTaqVersion(
				pathToTaq,
				this.i18,
				this.logHelper.showLog,
				this,
	...
	async getTaqBuild(): Promise<string | undefined> {
		try {
			const pathToTaq = await this.getTaqBinPath();
			return await Util.checkTaqBuild(
				pathToTaq,
				this.i18,
				this.logHelper.showLog,
				this,
	...
	async createTreeViews() {
		this.systemCheckTreeView = this.vscode.window.createTreeView<SystemCheckTreeItem>(
			'taqueria-system-check',
			{
	...
	exposeRefreshSandBoxDataCommand() {
		this.registerCommand(
			...
			async (item: SandboxTreeItemBase) => {
		...
		this.registerCommand(
			...
			async (treeViewItem?: SmartContractEntrypointTreeItem) => {
				...
				if (
					...
					|| !(item.parent.address ?? item.parent.alias)
				) {
	...
	async downloadAndInstallTaqCLI(): Promise<boolean> {
		...
		switch (process.platform) {
			case 'linux':
				return this.performDownloadingAndInstallTaqCLI(
					'https://taqueria.io/get/linux/taq',
				);
			case 'darwin':
				return this.performDownloadingAndInstallTaqCLI(
					'https://taqueria.io/get/macos/taq',
				);
			default:
				this.logHelper.showLog(
					OutputLevels.warn,
					`Taqueria CLI is not supported on ${process.platform}`,
				);
				return false;
		}
	}

	private async performDownloadingAndInstallTaqCLI(
		taqDownloadUrl: string,
	): Promise<boolean> {
		// 2. Download binary
		...
		if (curlError) {
			this.logHelper.showLog(
				OutputLevels.error,
				`Failed to download Taqueria CLI: ${curlError}`,
			);
			return false;
		}
		this.logHelper.showLog(
			OutputLevels.info,
			`Taqueria CLI downloaded successfully`,
		);

		// 3. Make Taqueria CLI executable
		const { executionError: chmodError } = await execCmd(
			`chmod +x /tmp/taq`,
			this.logHelper.showLog,
		);
		if (chmodError) {
			this.logHelper.showLog(
				OutputLevels.error,
				`Failed to make Taqueria CLI executable: ${chmodError}`,
			);
			return false;
		}
		this.logHelper.showLog(
			OutputLevels.info,
			`Taqueria CLI made executable successfully`,
		);
		...
		if (moveError) {
			this.logHelper.showLog(
				OutputLevels.error,
				`Failed to move Taqueria CLI to /usr/local/bin: ${moveError}`,
			);
			return false;
		}
		this.logHelper.showLog(
			OutputLevels.info,
			`Taqueria CLI moved to /usr/local/bin successfully`,
		);

		// 6. Check if taq is reachable
		const { executionError: checkError } = await execCmd(
			`taq --version`,
			this.logHelper.showLog,
		);
		if (checkError) {
			this.logHelper.showLog(
				OutputLevels.error,
				`Failed to check Taqueria CLI: ${checkError}`,
			);
			return false;
		}
		this.logHelper.showLog(
			OutputLevels.info,
			`Taqueria CLI installed successfully`,
		);
