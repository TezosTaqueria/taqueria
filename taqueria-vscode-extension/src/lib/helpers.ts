...
export const exposeInitTask = (context: vscode.ExtensionContext, output: vscode.OutputChannel, i18n: I18N, folders: readonly vscode.WorkspaceFolder[]) => {
...
export const exposeInstallTask = (context: vscode.ExtensionContext, output: vscode.OutputChannel, folders: readonly vscode.WorkspaceFolder[], i18n: I18N) => {
	...
	return exposeTask(Commands.install, (pathToTaq: Util.PathToTaq) =>
		...
		.then(results => {
			...
			if (results.length === 0) {
				...
				showError({code: 'E_NO_TAQUERIA_PROJECTS', msg: "You don't have any Taqueria projects. You'll need to taq'ify a project using \"Taqueria: Init\" before you can install a plugin."})
				)
			}
...
export const exposeTasksFromProject =
	(context: vscode.ExtensionContext, output: vscode.OutputChannel, folders: readonly vscode.WorkspaceFolder[], i18n: I18N) =>
...
export const exposeTasksFromState = (context: vscode.ExtensionContext, output: vscode.OutputChannel, folders: readonly vscode.WorkspaceFolder[], i18n: I18N) =>
	getTaqifiedDirectories(folders, i18n)
...
export const getSandboxNames = (projectDir: Util.TaqifiedDir) =>
