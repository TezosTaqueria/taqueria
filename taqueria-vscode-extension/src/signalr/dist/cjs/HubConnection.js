...
class HubConnection {
	...
	_stopInternal(error) {
		...
		this._cleanupTimeout();
		this._cleanupPingTimer();
		this._stopDuringStartError = error
			|| new Error('The connection was stopped before the hub handshake could complete.');
						...
						if (callback) {
							...
							try {
								...
								this._logger.log(
									...
									`Stream callback threw error: ${(0, Utils_1.getErrorString)(e)}`,
								);
							}
						}
						break;
	...
	_connectionClosed(error) {
		...
		this._stopDuringStartError = this._stopDuringStartError || error
		...
		Object.keys(callbacks)
			.forEach(key => {
				...
				try {
					...
					this._logger.log(
						ILogger_1.LogLevel.Error,
						`Stream 'error' callback called with '${error}' threw error: ${(0, Utils_1.getErrorString)(e)}`,
					);
				}
			});
	}
			...
			streams[streamId].subscribe({
				...
				error: err => {
					...
					promiseQueue = promiseQueue.then(() =>
						this._sendWithProtocol(this._createCompletionMessage(streamId, message))
					);
				},
				next: item => {
