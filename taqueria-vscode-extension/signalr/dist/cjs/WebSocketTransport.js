...
class WebSocketTransport {
		...
		return new Promise((resolve, reject) => {
			...
			webSocket.onmessage = message => {
				this._logger.log(
					...
					`(WebSockets transport) data received. ${(0, Utils_1.getDataDetail)(message.data, this._logMessageContent)}.`,
				);
				if (this.onreceive) {
					try {
	...
	send(data) {
		if (this._webSocket && this._webSocket.readyState === this._webSocketConstructor.OPEN) {
			this._logger.log(
				ILogger_1.LogLevel.Trace,
				`(WebSockets transport) sending data. ${(0, Utils_1.getDataDetail)(data, this._logMessageContent)}.`,
			);
	...
	_close(event) {
		...
		if (this.onclose) {
			if (this._isCloseEvent(event) && (event.wasClean === false || event.code !== 1000)) {
				this.onclose(
					new Error(`WebSocket closed with status code: ${event.code} (${event.reason || 'no reason given'}).`),
				);
