...
export const getPluginName = (input: string): NpmPluginName => {
    ...
    const retval = hasVersion ? input.substring(0, endIndex).trim() : input.trim()
    return retval as NpmPluginName
}

export const requireNPM = (projectDir: SanitizedAbsPath, i18n: i18n): Future<TaqError, Manifest> =>
    pipe(
        readJsonFile<Manifest>(projectDir.join('package.json').value),
        chainRej((previous) =>
            reject({ kind: 'E_NPM_INIT', msg: i18n.__('npmInitRequired'), context: projectDir, previous }),
        ),
    )

export const getPluginPackageJson = (pluginNameOrPath: string, projectDir: SanitizedAbsPath) =>
    pipe(
...
const addToPluginList = (pluginName: NpmPluginName, config: ConfigArgs) =>
    pipe(
        ...
        map((manifest: { name: string }) => {
            ...
            const updatedConfig = Object.entries(config).reduce((retval: Record<string, unknown>, [key, val]) => {
                if (['configFile', 'hash', 'configDir', 'projectDir'].includes(key)) return retval
...
export const installPlugin = (configDir: SanitizedPath, projectDir: SanitizedAbsPath, i18n: i18n, plugin: string) =>
    pipe(
...
export const uninstallPlugin = (configDir: SanitizedPath, projectDir: SanitizedAbsPath, i18n: i18n, plugin: string) =>
    pipe(
        requireNPM(projectDir, i18n),
        ...
        chain((config: ConfigArgs) => {
            ...
            return pipe(resolve({ ...config, plugins }), chain(writeJsonFile(config.configFile.value)))
        }),
